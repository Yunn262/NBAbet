...
- Usa requests.Session() para reaproveitar conexões (mais rápido quando
  chamamos vários endpoints seguidos, como na página de análise).
- Exceções tratadas de forma específica (rede vs. JSON inválido).
- Retry simples com backoff em erros de rede/5xx transitórios.
- Validação da api_key na criação do cliente.
"""
...
class FootballAPI:
    ...
    def close(self) -> None:
        """Fecha a sessão HTTP. Chamar quando o cliente já não for necessário."""
        self.session.close()
    ...
    def get(self, endpoint: str, params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        """Faz um GET à API, devolvendo sempre um dict.

        Em caso de erro, devolve {"success": False, "error": ...} em vez
        de deixar a exceção propagar — assim o resto da app não precisa
        ...
        for attempt in range(MAX_RETRIES + 1):
            try:
                ...
                last_error = f"Erro de ligação à API: {exc}"
            except requests.exceptions.RequestException as exc:
                last_error = f"Erro ao contactar a API: {exc}"
            else:
                # Erros 5xx costumam ser transitórios — vale a pena tentar de novo.
            ...
            # Erro de rede: espera antes de tentar de novo (exceto na última tentativa).

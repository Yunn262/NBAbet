...
AtualizaÃ§Ã£o importante: a API da footballdata.io exige o plano PRO para
/matches/{id}/predictions, /btts e /corners. No plano free, sÃ³
/matches/{id}/probabilities e /matches/{id}/stats estÃ£o disponÃ­veis â€” e
...
- Normaliza os dois formatos (Pro: match_winner.home/draw/away vs
  Free: home_win/draw/away_win) num Ãºnico formato interno.
- ExpÃµe is_plan_gated() para o app conseguir avisar o utilizador quando um
  mercado nÃ£o apareceu por limitaÃ§Ã£o de plano, em vez de mostrar
  "Dados insuficientes" sem explicaÃ§Ã£o.
...
def get_best_value_multi(dicts: Sequence[Dict[str, Any]], keys: Sequence[str]) -> Dict:
    """Como get_best_value, mas tenta vÃ¡rias chaves alternativas."""
    for key in keys:
...
def analisar_partida(
    ...
    return {
        ...
        # mostrar um aviso em vez de "Dados insuficientes" sem explicaÃ§Ã£o.
